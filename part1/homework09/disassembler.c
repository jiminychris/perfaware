...
u8 EA_DISPLACEMENT_ONLY = 6;
u8 EALookupNoDisplacement[] =
{
    7,
    8,
    8,
...
int main(int ArgCount, char **Args)
{
    ...
    if (2 == ArgCount)
    {
        ...
        u8 Byte0;
        size_t BytesRead = fread(Memory, 1, ArrayCount(Memory), MachineCodeFile);
        u32 TotalClocks = 0;
        while (InstructionPointer < BytesRead)
        {
            ...
            u8 SetFlags = 0;
            u32 IsSub = 0;
            Byte0 = Memory[InstructionPointer++];
            ...
            if (0xB0 == (Byte0 & 0xF0))
            {
                ...
                {
                    Data = (Memory[InstructionPointer++] << 8) | DataLo;
                    Registers[Reg] = Data;
                }
                else
                {
            ...
            {
                u8 Byte1 = Memory[InstructionPointer++];
                ...
                switch (Mod)
                {
                    case MODE_MEMORY_DISPLACEMENT_NONE:
                    {
                        ...
                        u32 EA = EALookupNoDisplacement[R_M];
                        switch (R_M) {
                    ...
                    case MODE_MEMORY_DISPLACEMENT_8_BIT:
                    {
                        ...
                        if (Disp)
                        {
                            ...
                            MemoryAddress = MemoryAddressBuffer;
                            Clocks += EALookupWithDisplacement[R_M];
                        }
                        else
                        {
                            Clocks += EALookupNoDisplacement[R_M];
                        }
                        switch (R_M) {
                            case memory_mode_BX_SI:
                            {
            ...
            {
                u8 DataSize = Byte0 & 1;
                u8 AddressLo = Memory[InstructionPointer++];
                u8 AddressHi = Memory[InstructionPointer++];
                ...
                switch (Mod)
                {
                    ...
                    case MODE_MEMORY_DISPLACEMENT_16_BIT:
                    {
                        ...
                        switch (OperatorIndex)
                        {
                            ...
                            {
                                Result = DestValue + SourceValue;
                                Clocks += (RegIsDest ? 9 : 16) + EALookupWithDisplacement[R_M];
                            } break;
            ...
            {
                s8 IPInc8 = Memory[InstructionPointer++];
                u32 Jump = 0;
                switch (Byte0)
                {
            ...
            TotalClocks += Clocks;
            printf(" ; Clocks: %+d = %u", Clocks, TotalClocks);
        ...
        snprintf(DumpFileName, sizeof(DumpFileName), "%s.data", FileName);
        FILE *DumpFile = fopen(DumpFileName, "wb");
        fwrite(Memory, 1, ArrayCount(Memory), DumpFile);
    }
    else
    {
